...
- [X] Instead of changing missiles, use the value `missiles - ready` directly in the second `println!(...)`
  ...
  mut is not needed, since we are using the value in a calculation and not assign it
- [X] Add another variable to your program *but don't use it*.
  - What does cargo say when you run your program?
    unused variable
- [X] Try modifying a constant in `main()` (for example, `READY_AMOUNT = 1`). What does the error look like? Broke
